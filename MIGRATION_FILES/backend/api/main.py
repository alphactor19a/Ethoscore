...
# CORS: allow local dev UIs and Chrome extensions
app.add_middleware(
    CORSMiddleware,
    allow_origins=[
        "http://localhost:3000",
        "http://127.0.0.1:3000",
        "http://localhost:5173",
        "*",  # Allows Chrome extensions and all origins (safe for production with public API)
    ],
...
def startup_event() -> None:
    ...
    try:
        ...
        # Debug: Print all MODEL_* environment variables
        model_env_vars = {k: v for k, v in os.environ.items() if k.startswith('MODEL_')}
        print(f"[startup] MODEL_* environment variables found: {list(model_env_vars.keys())}")
        if model_env_vars:
            for key in model_env_vars.keys():
                value = model_env_vars[key]
                # Only show first 50 chars of value for security
                print(f"[startup]   {key} = {value[:50]}...")
        else:
            print("[startup] WARNING: No MODEL_* environment variables found!")
        
        print(f"[startup] Checking for model files in: {base_dir}")
